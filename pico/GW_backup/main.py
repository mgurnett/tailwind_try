...
if __name__ == "__main__":
    ...
    while True:
        batt_count += 1
        print (f'battery_count: {batt_count}')
...
#             if len(temp_list) > MAX_STORAGE:
        ...
        if batt_count == 10:
            batt_count = 0
            battery = 3.2
            log = {"recorded": time_str, "battery": battery, "chain": ip}
            print (log)
            try:
                response = urequests.post(const.STATUS_URL, json=log)
                print (response)
                response.close()
            except:            
                menu_item (6, f'Battery: {battery}V')
            else:             
                menu_item (4, f"response code {response.status_code}")                
